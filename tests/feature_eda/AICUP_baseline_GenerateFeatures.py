...
def data_generate(datapath, tar_dir):
    ...
    pathlist_txt = datapath.glob('**/*.txt')
    for file in pathlist_txt:
        ...
        fname_path = '{fname}.csv'.format(fname = Path(file).stem)
        with open(str(tar_dir / fname_path), 'w', newline = '') as csvfile:
...
data_dir = Path('.') / 'data' / 'raw' / '39_Training_Dataset' / 'train_data'
tar_dir = Path('.') / 'data' / 'processed' / '39_Training_Dataset' / 'tabular_data_train'
os.makedirs(tar_dir, exist_ok=True)
data_generate(data_dir, tar_dir)
