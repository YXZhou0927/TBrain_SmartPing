...
class HoldRacketXGBGenerator(XGBoostBase):
    """慣用手任務的XGBoost特徵生成器"""
    ...
    def __init__(self, use_gpu=True):
        ...
        """
        from config import hold_racket_config as config
        super().__init__(
    ...
    def _load_config(self):
        """載入配置"""
        from config import hold_racket_config
        return hold_racket_config
    
    def _generate_meta_features(self, all_meta_features, oof_indices, oof_preds, unique_ids):
        """生成慣用手任務的meta-features
        ...
        """
        # 二元分類任務只保存一個類別的機率值
        col_name = f"{self.task_name}"
        meta_values = np.zeros(len(unique_ids))
        meta_values[oof_indices] = oof_preds[:, 0]  # 使用第一列作為機率值(正例機率)
    ...
    def _predict_with_models(self, models, weights, dtest, n_samples):
        ...
        """
        # 二元分類
        weighted_pred = np.zeros((n_samples, 2))
        for i, model in enumerate(models):
            if i < len(weights):
                # 獲取模型預測
                raw_pred = model.predict(dtest)
                # 注意機率順序 [1-p, p]
                pred = np.vstack((1 - raw_pred, raw_pred)).T
    ...
    def _process_predictions(self, test_meta_features, test_predictions, weighted_pred, test_unique_ids):
        ...
        test_meta_features[self.task_name] = weighted_pred[:, 0]  # 使用第一列作為正例機率
        
        # 保存到預測DataFrame
        test_predictions[self.task_column] = weighted_pred[:, 0]
    ...
    def _format_submission(self, test_predictions, test_unique_ids, sample_submission):
        ...
        submission[self.task_column] = test_predictions[self.task_column].round(4)
